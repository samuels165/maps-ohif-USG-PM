...
export class SelectTree extends Component<PropType> {
  ...
  constructor(props) {
    ...
    this.state = {
      ...
      descriptionTerm: this.props.items.length > 0 ? null : this.props.description,
      currentNode: null,
      value: null,
      focuseIndex: 0,
    };
        ...
        {this.props.searchEnabled && (
          <div className="flex w-full flex-col">
            ...
            <input
              ...
              type="text"
              className={`border-primary-main border-primary-main appearance-none rounded border bg-black bg-black py-2 pr-3 text-sm leading-tight shadow transition duration-300 hover:border-gray-500 focus:border-gray-500 focus:outline-none focus:outline-none ${inputLeftPadding}`}
              placeholder={this.props.items.length > 0 ? 'Search labels' : 'Enter description'}
              autoFocus={this.props.autoFocus}
              onChange={this.searchLocationsDesc}
              value={this.state.descriptionTerm ? this.state.descriptionTerm : ''}
              onKeyDown={this.handleKeyDown}
            />
          </div>
        )}
  ...
  searchLocationsDesc = evt => {
    this.setState({
      currentNode: null,
      descriptionTerm: evt.currentTarget.value,
    });
  };
